...
module Homebrew
  def pull2
    ...
    # Changed formulae across all patches
    all_changed_formulae = []
    ...
    ARGV.named.each do |arg|
      ...
      if fetch_bottles
        ...
        unless ARGV.include? "--no-publish"
          ...
          bintray_published_formulae.concat(published).uniq!
        end
      end
      ...
      all_changed_formulae.concat(changed_formulae).uniq!
    end

    # Verify bintray publishing after all patches have been applied
    Formulary.clear_formula_class_cache
  ...
  def publish_changed_formula_bottles(tap, changed_formulae)
    ...
    if bintray_creds[:user] && bintray_creds[:key]
      ...
      changed_formulae.each do |f|
        ...
        publish_bottle_files_to_bintray(f, bintray_creds)
  ...
  def maybe_pbcopy(text)
    pbcopy(text) unless ARGV.include? "--no-pbcopy"
  end
  ...
  def publish_bottle_files_to_bintray(f, creds)
  ...
  # Fetches a bottle for any platform, without falling back to source download.
  def fetch_bottle_any_platform(names)
    names.each do |fname|
      f = Formula[fname]
  ...
  # The URL for a given bottle
  # Opts:
  #  bottle_tag  - tag indicating platform of bottle. Defaults to current
  #                process's platform
  def bottle_url(f, opts = {})
    my_bottle_tag = opts[:bottle_tag] || bottle_tag
    my_bottle_tag ||= bottle_tag
    spec = f.stable
    bottle_spec = spec.bottle_specification
    filename = Bottle::Filename.create(f, my_bottle_tag, bottle_spec.revision)
    "#{bottle_spec.root_url}/#{filename}"
  end

  def bottle_url_any_platform(f)
    a_bottle_tag = f.stable.bottle_specification.collector.keys.first
    return nil if a_bottle_tag.nil?
    bottle_url(f, { :bottle_tag => a_bottle_tag })
  end
  ...
  class FormulaInfoFromJson
    ...
    def self.lookup(name)
      ...
      opoo "Failed getting info for #{name}" unless $?.success?
  ...
  def verify_bintray_published(formulae_names)
    ...
    HOMEBREW_CACHE.cd do
      formulae.each do |f|
        ...
        bottle_file_url = bottle_url_any_platform(f)

        jinfo = FormulaInfoFromJson.lookup(f.name)
        bottle_info = jinfo.bottle_info(jinfo.bottle_tags.first)
        opoo "No bottle defined in formula #{f.name}" unless bottle_info
...
# brew code stuff
#
# This is stuff that will go in the main Library/Homebrew code if this makes it
# in to the main repo

class Formulary
  # Clears the cached class definitions for named formulae, so subsequent
  # #factory lookups will reload the class definition.
  def self.clear_formula_class_cache
    FORMULAE.clear
  end
end
